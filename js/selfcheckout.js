...
const SELFCHECKOUT_FLOW = {
  download: {
    ...
    desc: "Sign up with your name, email, and phone in under 3 minutes.",
  ...
  enroll: {
    ...
    title: "Enroll your face and ID",

    desc: "Complete a 3D face scan and upload a government-issued ID. All biometric data is encrypted and securely stored on-device.",
  ...
  shop: {
    ...
    title: "Shop and Scan as usual.",

    desc: "The self-checkout experience stays exactly the same for the shopper.",
  ...
  look: {
    ...
    desc: "Identity is matched against the secure on-device template with 99.8% accuracy in under 2 seconds.",
  ...
  receipt: {
    ...
    title: "Payment complete. Receipt delivered.",

    desc: "Payment, receipt delivery, and audit logging happen automatically before the customer leaves.",
    ...
    imageAlt:
      "Checkout complete with digital receipt and customer leaving store",
  },
};
...
function initSelfcheckoutFlowTabs(root = document) {
  ...
  const iconWrap = container.querySelector(
    "[data-selfcheckout-flow-icon-wrap]",
  );
  ...
  container.querySelectorAll("[data-selfcheckout-flow-tab]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      container
        .querySelectorAll("[data-selfcheckout-flow-tab]")
        .forEach((b) => {
